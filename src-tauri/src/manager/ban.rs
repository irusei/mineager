...
impl Server {
    ...
    pub async fn ban_player(
        ...
        let entry = BanEntry {
            ...
            created: String::from("2020-03-04 20:19:19 +0100"),
            source: String::from("Server"),
            expires: String::from("forever"),
        };
        ...
        if entries
            .iter()
            .any(|e| e.name == entry.name || e.uuid == entry.uuid)
        {
            return Ok(());
        }
    ...
    pub fn unban_player(&self, entry: BanEntry) -> Result<(), Box<dyn std::error::Error>> {
        ...
        entries = entries
            .iter()
            .filter(|e| e.name != entry.name && e.uuid != entry.uuid)
            .cloned()
            .collect();
