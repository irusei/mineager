...
export function ServerProperties({ server }: ServerPropertiesProps) {
  const [serverProperties, setServerProperties] = useState<Map<
    string,
    string
  > | null>(null);
  ...
  const getBoolPropertyState = (key: string) =>
    serverProperties?.get(key) == "true";
  ...
  function applyProperties() {
    if (serverProperties != null)
      invoke("write_properties", {
        serverId: server.server.server_id,
        newProperties: propertiesMapToString(serverProperties),
      });
  }

  function reloadProperties() {
    invoke("read_properties_lines", { serverId: server.server.server_id }).then(
      (p) => {
        let properties = p as string[];
        if (properties.length > 0)
          setServerProperties(getMapFromProperties(properties));
        else setServerProperties(null);
      },
    );
  }
  ...
  if (serverProperties === null) return <NoPropertiesFound />;
  ...
  return (
    <>
          ...
          <div className="bg-bg-2 p-3">
            ...
            <div>
              {serverProperties.has("server-port") && (
                <SettingContainer
                  name={"Server Port"}
                  description={"The server will run on this port"}
                >
                  <Input
                    type={"number"}
                    placeholder={"25565"}
                    ...
                    onChange={(event) =>
                      modifyPropertyState("server-port", event.target.value)
                    }
                  />
                </SettingContainer>
              )}
              {serverProperties.has("max-players") && (
                ...
                >
                  <Input
                    ...
                    onChange={(event) =>
                      modifyPropertyState("max-players", event.target.value)
                    }
                  />
                </SettingContainer>
              )}
              {serverProperties.has("online-mode") && (
                <SettingContainer
                  name={"Online Mode"}
                  description={"Players must be authenticated to join"}
                >
                  <Switch
                    checked={getBoolPropertyState("online-mode")}
                    disabled={isOnline}
                    onChecked={(checked) =>
                      modifyBoolPropertyState("online-mode", checked)
                    }
                  />
                </SettingContainer>
              )}
          ...
          <div className="bg-bg-2 p-3">
            ...
            <div>
              {serverProperties.has("difficulty") && (
                ...
                >
                  <Select
                    value={serverProperties.get("difficulty")!}
                    options={["easy", "normal", "hard", "peaceful"]}
                    disabled={isOnline}
                    setValue={(newValue) =>
                      modifyPropertyState("difficulty", newValue)
                    }
                  />
                </SettingContainer>
              )}
              {serverProperties.has("hardcore") && (
                <SettingContainer
                  name={"Hardcore"}
                  description={"Perma-death mode"}
                >
                  <Switch
                    checked={getBoolPropertyState("hardcore")}
                    disabled={isOnline}
                    onChecked={(checked) =>
                      modifyBoolPropertyState("hardcore", checked)
                    }
                  />
                </SettingContainer>
              )}
              {serverProperties.has("gamemode") && (
                ...
                >
                  <Select
                    value={serverProperties.get("gamemode")!}
                    options={["survival", "creative", "adventure", "spectator"]}
                    disabled={isOnline}
                    setValue={(newValue) =>
                      modifyPropertyState("gamemode", newValue)
                    }
                  />
                </SettingContainer>
              )}
              {serverProperties.has("spawn-protection") && (
                <SettingContainer
                  ...
                  description={
                    "The size (in blocks) of the spawn protected area"
                  }
                >
                  <Input
                    type={"number"}
              ...
              {serverProperties.has("view-distance") && (
                <SettingContainer
                  name={"Allowed Render Distance"}
                  description={"The maximum render distance a player can have"}
                >
                  <Input
                    type={"number"}
                    placeholder={"10"}
                    ...
                    onChange={(event) =>
                      modifyPropertyState("view-distance", event.target.value)
                    }
                  />
                </SettingContainer>
              )}
              {serverProperties.has("simulation-distance") && (
                <SettingContainer
                  ...
                  description={
                    "How many chunks around the player the server updates"
                  }
                >
                  <Input
                    type={"number"}
              ...
              {serverProperties.has("level-name") && (
                ...
                >
                  <Input
                    ...
                    onChange={(event) =>
                      modifyPropertyState("level-name", event.target.value)
                    }
                  />
                </SettingContainer>
              )}
              {serverProperties.has("level-seed") && (
                ...
                >
                  <Input
                    ...
                    onChange={(event) =>
                      modifyPropertyState("level-seed", event.target.value)
                    }
                  />
                </SettingContainer>
              )}
              {serverProperties.has("level-type") && (
                <SettingContainer
                  name={"World Type"}
                  description={
                    "The type of the world. Make sure to escape colons"
                  }
                >
                  <Input
                    type={"text"}
                    ...
                    onChange={(event) =>
                      modifyPropertyState("level-type", event.target.value)
                    }
                  />
                </SettingContainer>
              )}
      ...
      <div
        className={
          "flex flex-row w-full space-x-4 bg-bg-2 border-t border-border p-2.5"
        }
      >
        <Button
