...
export function ServerWhitelist({ server }: ServerWhitelistProps) {
  ...
  useEffect(() => {
    ...
    invoke("is_whitelist_enabled", { serverId: server.server.server_id }).then(
      (enabled) => {
      ...
      <div className="relative min-h-120 max-h-120 p-3 overflow-y-scroll pb-10">
        ...
        <div className="mb-4">
          <SettingContainer
            ...
            description="Restrict who is allowed to join the server."
          >
            <Switch
              checked={whitelistEnabled}
              onChecked={(v) => {
                setWhitelistEnabled(v);
                invoke("set_whitelist_enabled", {
                  serverId: server.server.server_id,
                  enabled: v,
                });
              }}
          ...
          <SettingContainer
            name="Add Player"
            description="Add a player to the whitelist by their username."
          >
        ...
        <div className="flex items-center justify-between gap-2 mb-3">
          <div className="flex items-center gap-2">
            ...
            <p className="text-sm font-semibold text-text">
              Whitelisted Players ({entries.length})
            </p>
          </div>
        </div>
