...
export function ServerBans({ server }: ServerBansProperties) {
  ...
  async function refreshBanned() {
    invoke("read_banned_players", { serverId: server.server.server_id }).then(
      (banned) => setBannedPlayers(banned as PlayerBanEntry[]),
    );
    invoke("read_banned_ips", { serverId: server.server.server_id }).then(
      (banned) => setBannedIps(banned as IpBanEntry[]),
    );
  }
    ...
    <>
      ...
      <div className="flex-1 h-full min-h-120 max-h-120 bg-bg-2 flex flex-col">
        <div className="relative flex flex-col flex-1 overflow-hidden p-3 overflow-y-auto scrollbar-hide">
          ...
          <SettingContainer
            name="Ban Player"
            description="Enter the username of the player to ban."
          >
          ...
          <div className="flex-1 space-y-2 mb-6">
            {bannedPlayers.map((entry) => (
              <div
                key={entry.uuid}
                className="bg-bg-1 border border-border rounded-lg p-3 flex flex-col"
              >
                ...
                {entry.reason && (
                  <p className="text-[11px] text-text-2 pt-2">{entry.reason}</p>
                )}
              </div>
            ))}
          </div>
          ...
          <SettingContainer
            name="Ban IP"
            description="Enter the IP address to ban."
          >
            <div className="flex gap-2">
              <Input
                type="text"
