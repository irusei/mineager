export function sortVersions(versions: string[]) {
  ...
  return versions.sort((a, b) => {
    ...
    // -pre, -rc
    if (subvA.includes("-") && !subvB.includes("-")) return -1;
    if (!subvA.includes("-") && subvB.includes("-")) return 1;
    ...
    if (subvA.includes("-") && subvB.includes("-")) {
      ...
      if (suffixIndexA === -1 || suffixIndexB === -1) return 0; // what

      if (suffixIndexA > suffixIndexB) return 1;
      if (suffixIndexA < suffixIndexB) return -1;

      // if they're both pre or they're both rc, sort by version number at the end of suffix
      if (versionNumberA > versionNumberB) return 1;
    ...
    // 1.20.xx vs 1.20.yy
    if (parseInt(subvA) > parseInt(subvB)) return 1;
    if (parseInt(subvA) < parseInt(subvB)) return -1;
