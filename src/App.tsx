...
function App() {
  const [selectedServer, setSelectedServer] = useState<FrontendServer | null>(
    null,
  );
  ...
  useEffect(() => {
    const updateLocalServersUnlisten = listen(
  ...
  useEffect(() => {
    ...
    if (!found) setSelectedServer(null);
  }, [servers]);

  return (
    <div
      className={
        "flex flex-row min-w-screen min-h-screen max-w-screen max-h-screen bg-bg-1 overflow-hidden"
      }
    >
      <Sidebar
        servers={servers}
        selectedServer={selectedServer}
        onSelectedServer={(server: FrontendServer) => setSelectedServer(server)}
